...
def get_new_score_from_gpt(prompt: str, question: str, ground_truth: str, answer: str, model: str): #gpt-3.5-turbo
    ...
    message += f"标准答案：{ground_truth}\n"
    message += f"回答：{answer}\n"
    message += prompt
    completion = openai.ChatCompletion.create(
        engine = model, #gpt-3.5-turbo
        messages=[
        {"role": "system", "content": "你是一个公正的评价者。你始终需要给出评分。评分在0-1之间。评分应当严格按照给定格式：以<score>开始，以</score>结束。"},
    ...
    new_score = extract_score(response)
    return new_score[0]

def extract_score(s):
    #pattern = '<score>(.*?)</score>'
    pattern = r'(?:\d*\.\d+|\b[01]\b)'
    #result = re.search(pattern, s)
...
def get_prompt_score_from_model(prompt, file_path, iter_id = -1, sample_num = -1):
    ...
    gpt_score_list = calculate_similarity_by_gpt(prompt = prompt, question_list=question_list, truth_list=truth_list, outputs=answer_list)
...
def calculate_similarity_by_gpt(prompt, question_list, truth_list, outputs):
    ...
    for ques, output, truth in zip(question_list, outputs, truth_list):
        ...
        for _ in range(2):
            try:
                context = get_new_score_from_gpt(prompt=prompt,question=ques, ground_truth=truth, answer=output, model="gpt-35-turbo")
                #print("context =",find_floats_and_ints_in_string(context))
                ret_value = find_floats_and_ints_in_string(context)[0]
                if not 0 <= ret_value <= 1:
                    raise ValueError("评分范围不在0-1")
                score = ret_value
